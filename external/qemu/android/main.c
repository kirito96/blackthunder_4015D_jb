...
int main(int argc, char **argv)
{
    ...
    //Modified By Henry.Song 2013/04/24
    /* opts->custpack is never NULL (see createAVD) here */
    if (opts->custpack) {
        AFREE(hw->disk_custpackPartition_path);
        hw->disk_custpackPartition_path = ASTRDUP(opts->custpack);
    }
    else if (!hw->disk_custpackPartition_path[0]) {
        hw->disk_custpackPartition_path = avdInfo_getCustpackImagePath(avd);
        D("autoconfig: -custpack %s", hw->disk_custpackPartition_path);
    }
    ...
    if (opts->custpackdir != NULL) {
        if (!path_exists(opts->custpackdir)) {
            derror("Directory does not exist: %s", opts->custpackdir);
            exit(1);
        }
    }

    {
        char*  custpackImage   = NULL;
        char*  initImage = NULL;

        do {
            if (opts->custpack == NULL) {
                /* If -system is not used, try to find a runtime system image
                * (i.e. system-qemu.img) in the content directory.
                */
                custpackImage = avdInfo_getCustpackImagePath(avd);
                if (custpackImage != NULL) {
                    break;
                }
                /* Otherwise, try to find the initial custpack image */
                initImage = avdInfo_getDefaultCustpackImagePath(avd);
                if (initImage == NULL) {
                    derror("No initial custpack image for this configuration!");
                    exit(1);
                }
                break;
            }

            /* If -custpack <name> is used, use it to find the initial image */
            if (opts->custpackdir != NULL && !path_exists(opts->custpack)) {
                initImage = _getFullFilePath(opts->custpackdir, opts->custpack);
            } else {
                initImage = ASTRDUP(opts->custpack);
            }
            if (!path_exists(initImage)) {
                derror("Custpack image file doesn't exist: %s", initImage);
                exit(1);
            }
        ...
        if (custpackImage != NULL) {
            ...
            hw->disk_custpackPartition_path     = custpackImage;
            hw->disk_custpackPartition_initPath = NULL;
            D("Using direct custpack image: %s", custpackImage);
        } else if (initImage != NULL) {
            hw->disk_custpackPartition_path = NULL;
            hw->disk_custpackPartition_initPath = initImage;
            D("Using initial custpack image: %s", initImage);
        }

        /* Check the size of the custpack partition image.
        * If we have an AVD, it must be smaller than
        * the disk.custpackPartition.size hardware property.
        *
        * Otherwise, we need to adjust the custpackPartitionSize
        * automatically, and print a warning.
        *
        */
        const char* custpackImg = hw->disk_custpackPartition_path;
        uint64_t    custpackBytes;

        if (custpackImg == NULL)
            custpackImg = hw->disk_custpackPartition_initPath;

        if (path_get_size(custpackImg, &custpackBytes) < 0) {
            derror("Missing custpack image: %s", custpackImg);
            exit(1);
        }

        hw->disk_custpackPartition_size =
            _adjustPartitionSize("custpack", custpackBytes, defaultPartitionSize,
