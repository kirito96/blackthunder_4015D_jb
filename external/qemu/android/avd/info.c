 ...
 *   AVD
 *
 * - mandatory user data image ("userdata-qemu.img"), custpack image
 *   ("custpack.img") and cache image ("cache.img")
 *
...
//Modified By Henry.Song 2013/04/24
char*
avdInfo_getCustpackImagePath( AvdInfo*  i )
{
    const char* imageName = _imageFileNames[ AVD_IMAGE_CUSTPACK ];
    return _avdInfo_getContentFilePath(i, imageName);
}

char*
avdInfo_getDefaultCustpackImagePath( AvdInfo*  i )
{
    const char* imageName = _imageFileNames[ AVD_IMAGE_CUSTPACK ];
    return _getFullFilePath(i->contentPath, imageName);
}
