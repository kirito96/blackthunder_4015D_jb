...
help_build_images( stralloc_t*  out )
{
    ...
    "    - custpack.img   : the *initial* custpack image.\n"
    ...
    "  You can use the -sysdir, -system, -custpack, -kernel, -ramdisk, -datadir, -data options\n"
...
help_disk_images( stralloc_t*  out )
{
    ...
    "  options: -sysdir, -datadir, -kernel, -ramdisk, -system, -custpack, -data, -cache\n"
...
//Modified By Henry.Song 2013/04/24
static void
help_custpackdir(stralloc_t*  out)
{
    char   custpackdir[MAX_PATH];
    char   *p = custpackdir, *end = p + sizeof(custpackdir);

    p = bufprint_app_dir( p, end );
    p = bufprint( p, end, PATH_SEP "lib" PATH_SEP "images" );

    PRINTF(
    "  use '-custpackdir <dir>' to specify a directory where custpackdir\n"
    "  image files will be searched. on this system, the default directory is:\n\n"
    "      %s\n\n", custpackdir );
...
//Modified By Henry.Song 2013/04/24
static void
help_custpack(stralloc_t*  out)
{
    PRINTF(
    "  use '-custpack <file>' to specify the intial custpack image that will be loaded.\n"
    "  the default image is 'custpack.img' from the system directory.\n\n"

    "  NOTE: In previous releases of the Android SDK, this option was named '-image'.\n"
    "        And using '-custpack <path>' was equivalent to using '-custpackdir <path>' now.\n\n"
